...
def setup_logger_console_window(key="log"):
    ...
    # Very basic window.
    layout = [[sg.Output(size=(100, 10), key=key)], [sg.Button("Done", disabled=True)]]
...
def load_settings(settings_file, default_settings, values_key_2_settings_key):
    ...
    try:
        ...
        logger.info(
            "Load settings file: %s",
            {"settings_file": settings_file, "cfg_folder": settings["cfg_folder"]},
        )
...
def create_settings_window(settings, values_key_2_settings_key, app_version):
    ...
    layout = [
        ...
        TextInput("URL"),
        [
            TextLabel("Log Folder"),
            sg.Input(key="-LOG_PATH-", size=(62, 1)),
            sg.FolderBrowse(target="-LOG_PATH-"),
        ],
        [
            TextLabel("Log to console:"),
            sg.Checkbox("", default=False, key="-LOG_CONSOLE-"),
        ],
        [TextLabel("Log to file:"), sg.Checkbox("", default=True, key="-LOG_FILE-")],
        [
            TextLabel("Clear log on startup:"),
            sg.Checkbox("", default=False, key="-CLEAR_LOGS-"),
        ],
...
def reinitialize_logging(vargs, settings, parent):
    ...
    if value_diff:
        logger.info(
            "Calling to change logging settings: %s",
            {
        ...
        parent.initialize_logging(
            settings["log_path"], settings["log_console"], settings["log_file"]
        )
        logger.info(
            "Logging settings changed: %s",
            {"old_log_path": vargs["log_path"], "new_log_path": settings["log_path"]},
        )
...
def clear_logs(vargs, settings, p_get_log_filename=None):
    ...
    if p_get_log_filename:
        logger.info(
            "Set parent_get_log_filename to passed in function: %s", p_get_log_filename
        )
    ...
    if not (vargs.get("clear_logs", False) or settings.get("clear_logs", False)):
        if False:
            ...
            logger.warning(
                "REMOVE THIS: %s", {"settings": settings, "msg": "clear logs not set"}
            )
        return logs_cleared
    ...
    while True:
        ...
        if event == "Save":
            ...
            if values["-SETTINGS_FILE-"] != settings["settings_file"]:
                ...
                if not Path(values["-SETTINGS_FILE-"]).parent.is_dir():
                    ...
                    window.Refresh()
                    sg.popup(
                        "Improper path to file settings",
                        "Correct or try using the SaveAs button",
                        "Data not saved",
                    )
                    continue
        ...
        if event == "Edit":
            ...
            try:
                ...
                logger.error(e)
                logger.info(
                    "Editted settings file could not be loaded - restoring prior version"
                )

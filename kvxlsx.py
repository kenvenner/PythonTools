...
def getExcelCellFont(excel_dict: dict, row: int, col_name: str, debug: bool=False) -> tuple[str | None, str | None,  str | None, str | None]:
    """
    row - integer
    col_name - name of the column of interest
    ...
    if excel_dict['xlsxfiletype']:
        # get font settings
        cell_font = excel_dict['s'].cell(row=row + 1 + excel_dict['row_header'], column=col + 1).font
        # debugging
        if debug:
            print('getExcelCellFont:name:', cell_font.name,
                  'getExcelCellFont:size:', cell_font.size,
                  'getExcelCellFont:strike:', cell_font.strike)
        
        # return cell_font
        return cell_font.name, \
            cell_font.size,\
            cell_font.bold,\
            cell_font.italic,\
            cell_font.underline,\
        ...
        logger.error('feature not supported on xls file - only XLSX')
        print('kvxls:getExcelCellFont:feature not supported on xls file - only XLSX')
        raise


# routine to set a cell fill pattern
def setExcelCellFont(excel_dict: dict, row: int, col_name: str, \
        name: str|None=None, size: str|None=None, bold:bool|None=None, \
        italic: str|None=None, underline: str|None=None, strike: str|None=None, color: str|None=None, debug: bool=False):
    """
    excel_dict
    ...
    if excel_dict['xlsxfiletype']:
        # get the cell
        cell = excel_dict['s'].cell(row=row + 1 + excel_dict['row_header'], column=col + 1)
...
def readxls_excelDict(xlsfile: str, req_cols: list[str], xlatdict: dict | None=None, optiondict: dict | None=None, col_aref: list[str] | None=None, data_only: bool=True, debug: bool=False) -> dict[str, list[str]]:
    ...
    if 'save_colfmt' in optiondict: save_colfmt = optiondict['save_colfmt']
...
def excelDict2list_findheader(excel_dict: dict, req_cols: list[str], xlatdict: dict | None=None, optiondict: dict | None=None, col_aref: list[str] | None=None, debug: bool=False) -> list[dict]:
    ...
    for row in range(row_data_start, sheetmaxrow):
        ...
        if aref_result:
            ...
            if save_colfmt:
                cell_color, cell_fill_type, cell_start_color, cell_end_color = getExcelCellPatternFill(excel_dict, row, save_colfmt, debug=debug)
                cell_font_name, cell_font_size, cell_font_bold, cell_font_italic, cell_font_underline, cell_font_strike, cell_font_color = getExcelCellFont(excel_dict, row, save_colfmt, debug=debug)
    ...
    for record in data:
        ...
        if col_aref and len(col_aref):
            for xlscol in range(0, len(col_aref)):
                ...
                if xlsxfiletype:
                    if value is not None and not isinstance(value, (str, int, float, datetime.datetime)):

...
def grep_function_parse_line(
    line: str, func_found: bool, func_def: list, func_lines: list
) -> bool:
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Function Test Coverage for Python and Python-Test files"
    )
    parser.add_argument("file", help="filename we evaluate")
    parser.add_argument("--test", "-t", help="test filename associated with filename")
    parser.add_argument("--debug", "-d", action="store_true", help="Debugging outputs")
    parser.add_argument(
    ...
    for k in sorted(file_func.keys()):
        if k not in test_func and "test_" + k not in test_func:
            not_tested += 1
            if not print_hdr:
    ...
    for k in sorted(test_func.keys()):
        if k not in file_func:
            print(
                f"{vargs['test']}:{k} - tests non-existing function in {vargs['file']}"
            )
            test_no_func += 1
    if test_no_func:
        print(
            f"{vargs['file']}:{test_no_func} of {len(test_func)} functions test non-existing functions"
        )

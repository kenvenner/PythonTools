...
class TestKVGmailRcv(unittest.TestCase):
    ...
    def test_GmailImap_init_p04_user_valid_password_verbose(self):
        if debug:
            print("test_GmailImap_init_p04_user_valid_password_verbose:start")
        # verbose needs to remain True below
        imap = kvgmailrcv.GmailImap(
            {"verbose": True, "user": user, "password": password}
        )
        ...
        self.assertTrue(isinstance(imap.imapobj, imaplib.IMAP4_SSL))
        if debug:
            print("test_GmailImap_init_p04_user_valid_password_verbose:end")

    def test_GmailImap_init_p05_user_valid_password_invalid_folder_verbose(self):
        if debug:
            print(
                "test_GmailImap_init_p05_user_valid_password_invalid_folder_verbose:start"
            )
        # verbose needs to remain True below
        imap = kvgmailrcv.GmailImap(
            {
        ...
        self.assertEqual(
            imap.errmsg,
            "select_folder:folder-selection-problem:[b'[NONEXISTENT] Unknown Mailbox: invalid (Failure)']",
        )
        self.assertEqual(imap.user, user)
        self.assertEqual(imap.password, password)
        self.assertEqual(imap.verbose, True)
        ...
        if debug:
            print(
                "test_GmailImap_init_p05_user_valid_password_invalid_folder_verbose:end"
            )
    ...
    def test_GmailImap_init_p07_simple_include_exclude_lists(self):
        imap = kvgmailrcv.GmailImap(
            {
                "fromIncludeEmails": ["debbie_venner@yahoo.com"],
                "fromIncludeDomains": ["vennerllc.com"],
                "fromExcludeIfNotInclude": False,
                "fromExcludeEmails": ["ken_venner@yahoo.com"],
                "fromExcludeDomains": ["spacex.com"],
            }
        )
        self.assertEqual(imap.loggedIn, False)
        self.assertEqual(imap.folder_current, None)
    ...
    def test_GmailImap_init_p08_simple_include_exclude_strings(self):
        if debug:
            print("GmailImap_init_p08_simple_include_exclude_strings")
        imap = kvgmailrcv.GmailImap(
            {
                "fromIncludeEmails": "debbie_venner@yahoo.com",
                "fromIncludeDomains": "vennerllc.com",
    ...
    def test_GmailImap_login_p01_select_folder(self):
        if debug:
            print("test_GmailImap_login_p01_select_folder(self)")
        imap = kvgmailrcv.GmailImap(
            {
                "verbose": False,
                "user": user,
                "password": password,
                "imap_folder": folder_msgs,
            }
        )
        imap.select_folder(folder_msgs)
        if debug:
            print("imap.mail_ids", imap.mail_ids)
        self.assertEqual(imap.errmsg, "")
    ...
    def test_GmailImap_login_p01_getNextMessage(self):
        ...
        if debug:
            print("dump the mparse variable")
    ...
    def test_GmailImap_login_p02_getNextMessage_no_message(self):
        if debug:
            print("test_GmailImap_login_p02_getNextMessage_no_message")
        imap = kvgmailrcv.GmailImap(
            {
                "verbose": False,
                "user": user,
                "password": password,
                "imap_folder": folder_NoMsgs,
            }
        )
        imap.getNextMessage()
        if debug:
            print("dump the mparse variable")
        kvgmailrcv.dump_mparse(
            imap.mparse, "test_GmailImap_login_p02_getNextMessage_no_message"
        )
        if debug:
            print("dump is done")
        self.assertEqual(imap.errmsg, "")
    ...
    def test_GmailImap_setIncludeExclude_p02_strings(self):
        ...
        if debug:
            print("imap.fromIncludeEmails", imap.fromIncludeEmails)
        self.assertEqual(imap.fromIncludeEmails, ["ken@vennerllc.com"])
        self.assertEqual(imap.fromIncludeDomains, ["vennerllc.com"])
        self.assertEqual(imap.fromExcludeIfNotInclude, False)
        self.assertEqual(imap.fromExcludeEmails, ["kvenner@spacex.com"])
    ...
    def test_GmailImap_excludeMessage_p01_simple(self):
        if debug:
            print("test_GmailImap_excludeMessage_p01_simple(self)")
        imap = kvgmailrcv.GmailImap(
            {
                "verbose": False,
                "user": user,
    ...
    def test_GmailImap_excludeMessage_p02_ExcludeIfNotInclude(self):
        if debug:
            print("test_GmailImap_excludeMessage_p02_ExcludeIfNotInclude(self)")
        imap = kvgmailrcv.GmailImap(
            {
                "verbose": True,
                "user": user,

...
class TestKVIncver(unittest.TestCase):
    def test_build_version_update_p01_found_not_found_earlier(self):
        ...
        for line in build_match_lines.split("\n"):
            ...
            (
                line_new,
                bld_version_found_new,
                bld_version_old,
                bld_version_new,
                new_bld_version_new,
            ) = k.build_version_update(line, bld_version_found, new_bld_version)
    ...
    def test_major_minor_version_update_p01_match_version_found_version_(self):
        for line in major_minor_match_lines_version.split("\n"):
            ...
            (
                ...
                new_app_ver_new,
                version_changed_new,
            ) = k.major_minor_version_update(
                line, new_app_ver, version_found, opt_ver_found, debug=False
            )
    ...
    def test_major_minor_version_update_p02_match_version_found_at_version(
        self,
    ):
    ...
    def test_major_minor_version_update_p03_match_optversion_found(self):
    ...
    def test_major_minor_version_update_p04_match_optvalue_found(self):
    ...
    def test_major_minor_version_update_p05_match_version_found_next_line(self):
        for line in major_minor_match_lines_optvalue.split("\n"):
            version_found = True
            opt_ver_found = True
            new_app_ver = NEWAPPVERSIONMINOR
    ...
    def test_major_minor_version_update_p06_match_appversion_found_version_(
        self,
    ):
    ...
    def test_major_minor_version_update_p11_match_version_not_found_version_(
        self,
    ):
        for line in major_minor_match_lines_version.split("\n"):
            version_found = False
    ...
    def test_major_minor_version_update_p12_match_version_not_foundat_version(
        self,
    ):
        for line in major_minor_match_lines_at_version.split("\n"):
            version_found = False
    ...
    def test_major_minor_version_update_p13_match_optversion_not_found(self):
        for line in major_minor_match_lines_optversion.split("\n"):
            version_found = False
            opt_ver_found = False
            new_app_ver = ""
    ...
    def test_major_minor_version_update_p14_match_optvalue_not_found(self):
        for line in major_minor_match_lines_optvalue.split("\n"):
            version_found = False
            opt_ver_found = True
            new_app_ver = ""
    ...
    def test_major_minor_version_update_p15_match_version_not_found_next_line(
        self,
    ):
        for line in major_minor_match_lines_optvalue.split("\n"):
            version_found = False
    ...
    def test_major_minor_version_update_p16_match_appversion_not_found_version_(
        self,
    ):
        for line in major_minor_match_lines_appversion.split("\n"):
            version_found = False
    ...
    # ----------------------------------------
    def test_build_version_update_p01_dblq_found_not_found_earlier(self):
        # for each line type - we find one instance of the buildversion line and update it
        # and we HAVE NEVER seen a buildversoin line before
        # so new_bld_version is passed in as an empty
    ...
    def test_build_version_update_p02_dblq_found_earlier(self):
        # for each line type - we find one instance of the buildversion line and update it
        # and we HAVE seen a buildversoin line before
        # so new_bld_version is passed in as the prior found value
        for line in build_match_lines_dblq.split("\n"):
    ...
    def test_build_version_update_p03_dblq_found_earlier_different_version(
        self,
    ):
        for line in build_match_lines_dblq.split("\n"):
            bld_version_found = True
    ...
    def test_build_version_update_p04_dblq_not_found_not_found_earlier(self):
        line = "Not a line that should match"
        bld_version_found = False
        new_bld_version = ""
        # first run to change
    ...
    def test_build_version_update_p05_dblq_not_found_found_earlier(self):
        line = "Not a line that should match"
        bld_version_found = True
        new_bld_version = UPDATEDBUILDVERSION
        # first run to change
    ...
    def test_major_minor_version_update_p01_dblq_match_version_found_version_(
        self,
    ):
    ...
    def test_major_minor_version_update_p02_dblq_match_version_found_at_version(
        self,
    ):
    ...
    def test_major_minor_version_update_p03_dblq_match_optversion_found(self):
    ...
    def test_major_minor_version_update_p04_dblq_match_optvalue_found(self):
    ...
    def test_major_minor_version_update_p05_dblq_match_version_found_next_line(
        self,
    ):
        for line in major_minor_match_lines_optvalue_dblq.split("\n"):
            version_found = True
    ...
    def test_major_minor_version_update_p06_dblq_match_appversion_found_version_(
        self,
    ):
    ...
    def test_major_minor_version_update_p11_dblq_match_version_not_found_version_(
        self,
    ):
        for line in major_minor_match_lines_version_dblq.split("\n"):
            version_found = False
    ...
    def test_major_minor_version_update_p12_dblq_match_version_not_foundat_version(
        self,
    ):
        for line in major_minor_match_lines_at_version_dblq.split("\n"):
            version_found = False
    ...
    def test_major_minor_version_update_p13_dblq_match_optversion_not_found(
        self,
    ):
        for line in major_minor_match_lines_optversion_dblq.split("\n"):
            version_found = False
    ...
    def test_major_minor_version_update_p14_dblq_match_optvalue_not_found(self):
        for line in major_minor_match_lines_optvalue_dblq.split("\n"):
            version_found = False
            opt_ver_found = True
            new_app_ver = ""
    ...
    def test_major_minor_version_update_p15_dblq_match_version_not_found_next_line(
        self,
    ):
        for line in major_minor_match_lines_optvalue_dblq.split("\n"):
            version_found = False
    ...
    def test_major_minor_version_update_p16_dblq_match_appversion_not_found_version_(
        self,
    ):
        for line in major_minor_match_lines_appversion_dblq.split("\n"):
            version_found = False
    ...
    def test_update_file_version_p01_minor_app(self):
        for line in file_create_test_lines.split("\n"):
            ...
            with open(file_tmp, "w") as f:
                ...
                new_file_expected_content = (
                    file_create_header_lines
                    + line.replace("1.2", "1.03")
                    + file_create_footer_lines
                )
            major_update = False
            build_only = False
        ...
        for line in file_create_test_lines.split("\n"):
            ...
            with open(file_tmp, "w") as f:
                ...
                new_file_expected_content = (
                    file_create_header_lines
                    + line
                    + file_create_footer_lines.replace("9", "10")
                )
            major_update = False
            build_only = True
            test = False
    ...
    # ----------------------------------------
    def test_update_file_version_p01_dblq_minor_app(self):
    ...
    def test_update_file_version_p02_dblq_major_app(self):
        for line in file_create_test_lines_dblq.split("\n"):
            # create a file to be updated
            file_tmp = "kvincver_test.txt"
            with open(file_tmp, "w") as f:
    ...
    def test_update_file_version_p03_dblq_buildversion_app(self):
        for line in file_create_test_lines_dblq.split("\n"):
            # create a file to be updated
            file_tmp = "kvincver_test.txt"
            with open(file_tmp, "w") as f:
    ...
    ########################################
    # the function name: def handle_exception(exc_type, exc_value, exc_traceback):
    def test_handle_exception_p01_pass(self):
        pass

    ########################################
    # the function name: def debug_print(
    def test_debug_print_p01_pass(self):
        pass

    ########################################
    # prior function: update_file_version
    # the function name: def git_modified_files_in_folder(folder_path="", debug=False):
    def test_git_modified_files_in_folder_p01_pass(self):
        pass

    ########################################
    # the function name: def version_changed_in_git_branch(filename, debug=False):
    def test_version_changed_in_git_branch_p01_pass(self):
        pass

...
def save_and_log_dbms_extract(
    ...
    with open(log_file_path + log_filename, "a") as fp:
        ...
        # output results
        fp.write(
            f"{excel_file_path},{now.isoformat()},{len(result.index)},{(endtime - starttime) / 60},{(save_starttime - starttime) / 60},{(endtime - save_starttime) / 60}, {proc_percent}\n"
        )


def save_and_log_exception_rpt(
    excel_file_path,
    result,
    starttime,
    now,
    ...
    if cc_cfg:
        ...
        cc_optiondict = kvutil.kv_parse_command_line(
            copy_comments.optiondictconfig,
            cmdlineargs=cc_args,
            skipcmdlineargs=True,
        )
        ...
        for src_fullfilename in cc_src_files:
            ...
            if cc_optiondict["rmv_fname"]:
                rmv_data, dst_lookup = copy_comments.removed_records(
                    src_data, dst_data, cc_optiondict
                )
            else:
                rmv_data = []
            ...
            if cc_optiondict["add_fname"]:
                add_data = copy_comments.added_records(
                    src_data, dst_data, src_lookup, cc_optiondict
                )
            else:
                add_data = []
            ...
            copy_comments.generate_out_output_file_not_formatted(
                dst_data, cc_optiondict, updated_recs
            )
            copy_comments.generate_rmv_output_file_not_formatted(
                rmv_data, cc_optiondict
            )
            copy_comments.generate_add_output_file_not_formatted(
                add_data, cc_optiondict
            )
    ...
    # open the log file and output record
    with open(os.path.join(log_file_path, log_filename), "a") as fp:
        # create header if the file did not exist
        if not log_exists:
            fp.write(
                ",".join(
        ...
        # output results
        fp.write(
            f"{excel_file_path},{now.isoformat()},{len(result)},{(endtime - starttime) / 60}\n"
        )


def save_lot_serial_csv_exception_rpt(
    excel_file_path,
    result,
    lotfield="islotitem",
    outflds=None,
    ...
    if result and (outflds or fldmapping):
        ...
        if bad_outflds:
            ...
            raise Exception(
                "Outflds defined not in the record: " + ",".join(bad_outflds)
            )
...
def set_master_dir_strings():
    ...
    master_dir = sp_synched_dir_path(
        "/NetSuite Implementation - Documents/Master Mapping Files/"
    )
    master_output_dir = sp_synched_dir_path(
        "/NetSuite Implementation - Documents/Master Mapping Files/output/static/"
    )
    master_static_dir = sp_synched_dir_path(
        "/NetSuite Implementation - Documents/Master Mapping Files/output/static/"
    )
    master_mnfro_dir = sp_synched_dir_path(
        "/NetSuite Implementation - Documents/Master Mapping Files/Manufacturo DataSets/"
    )
    master_ns_dir = sp_synched_dir_path(
        "/NetSuite Implementation - Documents/Master Mapping Files/NetSuite DataSets/"
    )
    master_error_dir = sp_synched_dir_path(
        "/NetSuite Implementation - Documents/Master Mapping Files/error/"
    )

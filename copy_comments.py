...
# RE to find environment variables in a string
RE_ENV_VAR = re.compile(r"(\%(.+)\%)")


# ----------------------------------------


def expand_dir_env_var(optiondict: dict, dir: str) -> None:
    """
    Update the string for "dir" key passed in
    extract the environment varilable defined by %<some_value%
    look this up and replace the %<some_value>% with teh
    the actual setting for that env var

    """

    # not in the dictionary - no work
    if dir not in optiondict:
        return

    # not populated - no work
    if not optiondict[dir]:
        return

    # check for the variable
    m = RE_ENV_VAR.search(optiondict[dir])

    # nothing to update - no work
    if not m:
        return

    # capture the value of the environment variable
    env_var_value = os.environ.get(m.group(2))

    # we did not find a value - create error
    if not env_var_value:
        # we could not find this value - we have an issue
        raise ValueError(
            f"Did not find env var [{m.group(2)}] in [{dir}] line: {optiondict[dir]}"
        )
...
def validate_inputs(optiondict: dict) -> bool | None:
    ...
    # expand out directory variables
    for dir in [k for k in optiondict.keys() if "_dir" in k]:
        expand_dir_env_var(optiondict, dir)
    ...
    if optiondict["default_fname"] not in (
        "dst_fname",
        "src_fname",
        "out_fname",
    ):
        if optiondict["disp_msg"]:
            print(
                "default_fname must be one of the following: "
                + ",".join(["dst_fname", "src_fname", "out_fname"])
                + " but is: "
    ...
    if not optiondict.get(optiondict["default_fname"]):
        ...
        optiondict["error_msg"] = (
            f"default_fname [{optiondict['default_fname']}] is not set"
        )
        return False
    ...
    for fld in ["rmv", "add", "out"]:
        ...
        if optiondict[fld + "_fname_append"]:
            ...
            optiondict[fld + "_fname"] = (
                fname + optiondict[fld + "_fname_append"] + fext
            )
        ...
        if optiondict[fld + "_fname_uniqtype"]:
            ...
            file_href = {
                ...
                "filename": os.path.join(
                    optiondict[fld + "_dir"], optiondict[fld + "_fname"]
                ),
                "uniqtype": optiondict[fld + "_fname_uniqtype"],
                "forceuniq": True,
            }
            optiondict[fld + "_fname"] = os.path.basename(
                kvutil.filename_unique(filename_href=file_href, debug=False)
            )
    ...
    if optiondict["src_fname_glob"]:
        optiondict["src_fname"] = os.path.basename(
            kvutil.filename_maxmin(
                optiondict["src_dir"] + optiondict["src_fname_glob"],
                optiondict["src_fname_glob_rev"],
            )
        )
    # deal with dst_fname_glob
    if optiondict["dst_fname_glob"]:
        optiondict["dst_fname"] = os.path.basename(
            kvutil.filename_maxmin(
                optiondict["dst_dir"] + optiondict["dst_fname_glob"],
                optiondict["dst_fname_glob_rev"],
            )
        )

    # format/output options
    if (
        "src_width" in optiondict
        and optiondict["src_width"]
        and (optiondict.get("col_width") or optiondict.get("col_hidden"))
    ):
        # output messages
        if optiondict["disp_msg"]:
            print(
                "You should not have src_width true and values in col_width or col_hidden"
            )
        optiondict["error_msg"] = (
    ...
    if type(optiondict["key_fields"]) is not list:
        ...
        if optiondict["disp_msg"]:
            print(
                "Attribute key_fields must be a list but is a: ",
                type(optiondict["key_fields"]),
            )
        optiondict["error_msg"] = (
            "Attribute key_fields must be a list but is a: ",
            type(optiondict["key_fields"]),
        ...
        for idx, copydict in enumerate(optiondict["internal_copy_fields"]):
            for fld in ["src", "dst"]:
                if fld not in copydict:
                    print(
                        f"internal_copy_fields [{idx + 1}] missing required key [{fld}]: "
                        + str(copydict)
                    )
                    has_issues = True
    ...
    if optiondict["internal_compare_fields"]:
        ...
        for idx, copydict in enumerate(optiondict["internal_compare_fields"]):
            for fld in ["src", "dst"]:
                if fld not in copydict:
                    print(
                        f"internal_compare_fields [{idx + 1}] missing required key [{fld}]: "
                        + str(copydict)
                    )
                    has_issues = True
            # set the default if not set
    ...
    if optiondict["force_copy_flds"]:
        if "copy_fields" not in optiondict:
            ...
            if optiondict["disp_msg"]:
                print(
                    "When [force_copy_flds] is set you must defined [copy_fields] and have not"
                )
            optiondict["error_msg"] = (
                "When [force_copy_flds] is set you must defined [copy_fields] and have not"
            )
            ...
            if optiondict["disp_msg"]:
                print(
                    "When [copy_fields] is defined it must be a list and is:  ",
                    type(optiondict["copy_fields"]),
                )
            optiondict["error_msg"] = (
                "When [copy_fields] is defined it must be a list and is:  ",
                type(optiondict["copy_fields"]),
    ...
    if optiondict.get("internal_copy_fields"):
        ...
        if not isinstance(optiondict["internal_copy_fields"], list):
            ...
            if optiondict["disp_msg"]:
                print(
                    "internal_copy_fields must be of type list of dicts and is of type: "
                    + str(type(optiondict["internal_copy_fields"]))
                )
            optiondict["error_msg"] = (
                "internal_copy_fields must be of type list of dicts and is of type: "
                + str(type(optiondict["internal_copy_fields"]))
    ...
    if optiondict.get("internal_compare_fields"):
        ...
        if not isinstance(optiondict["internal_compare_fields"], list):
            ...
            if optiondict["disp_msg"]:
                print(
                    "internal_compare_fields must be of type list of dicts and is of type: "
                    + str(type(optiondict["internal_compare_fields"]))
                )
            optiondict["error_msg"] = (
                "internal_compare_fields must be of type list of dicts and is of type: "
                + str(type(optiondict["internal_compare_fields"]))
...
def load_records(
    optiondict: dict, srctype: str = "src", disp_msg: bool = False
) -> list[dict]:
    """
    ...
    full_filename = os.path.join(
        optiondict[srctype + "_dir"], optiondict[srctype + "_fname"]
    )
    ...
    if optiondict["ignore_missing_" + srctype]:
        ...
        if not os.path.exists(full_filename):
            if disp_msg:
                print(
                    "copy_comments:load_records:file does not exist: "
                    + str(full_filename)
                )
            return []
    ...
    loaded_optiondict = {
        "sheetname": optiondict[srctype + "_ws"],
        "save_row": True,
    }
    ...
    if srctype + "_reqcols" in optiondict and optiondict[srctype + "_reqcols"]:
        loaded_data = kvxls.readxls2list_findheader(
            full_filename,
            optiondict[srctype + "_reqcols"],
            optiondict=loaded_optiondict,
        )
    else:
        # load with the assumption that the first row is the header
        loaded_data = kvxls.readxls2list(
            full_filename,
            optiondict[srctype + "_ws"],
            optiondict=loaded_optiondict,
        )
...
def read_src_file_format(optiondict: dict) -> None:
    ...
    if "src_width" in optiondict and optiondict["src_width"]:
        ...
        # open once the xlsx
        ws, wb = kv_excel.open_xlsx_get_ws_wb(
            full_filename,
            ws_sheetname=optiondict.get("src_ws", None),
            disp_msg=disp_msg,
        )
        ...
        optiondict["col_width"] = kv_excel.get_existing_column_width_ws(
            ws, disp_msg=disp_msg
        )
        ...
        optiondict["col_hidden"] = kv_excel.get_existing_column_hidden_ws(
            ws, disp_msg=disp_msg
        )
        ...
        if optiondict["get_col_fmt"]:
            if disp_msg:
                print(
                    "Getting col_format formatting from src_fname for columns: "
                    + ",".join(optiondict["get_col_fmt"])
                )
            optiondict["col_format"] = kv_excel.get_existing_column_format_ws(
                ws, columns=optiondict["get_col_fmt"], disp_msg=disp_msg
            )

        # rationalize the values - remove from width those that are not hiddne
        optiondict["col_width"] = {
            k: v
            for k, v in optiondict["col_width"].items()
            if k not in optiondict["col_hidden"]
        }
...
def validate_missing_columns(
    loaded_data: list[dict], optiondict: dict, fld: str
) -> list:
    """
    ...
    return [
        x
        for x in optiondict[fld]
        if not isinstance(x, dict) and x not in loaded_data[0]
    ]
...
def src_to_dst_actions(
    src_data: list, dst_data: list, optiondict: dict
) -> tuple[int, int, dict]:
    """
    ...
    if (
        not optiondict["copy_fields"]
        and not optiondict["internal_copy_fields"]
        and not optiondict["internal_compare_fields"]
    ):
...
def removed_records(
    ...
    if type(optiondict["key_fields"]) is not list:
        raise TypeError(
            "key_fields defined and not list but type: "
            + str(type(optiondict["key_fields"]))
        )
...
def added_records(
    ...
    add_data = kvutil.extract_unmatched_data(
        dst_data, src_lookup, optiondict["key_fields"]
    )

    return add_data


def generate_out_output_file_not_formatted(
    dst_data: list[dict], optiondict: dict, updated_recs
) -> None:
    """
...
def generate_rmv_output_file_not_formatted(
    rmv_data: list[dict], optiondict: dict
) -> None:
    """
...
def generate_add_output_file_not_formatted(
    add_data: list[dict], optiondict: dict
) -> None:
    """
...
def format_output(optiondict):
    ...
    if optiondict.get("create_json"):
        ...
        output = {
            ...
            "copy_fields": [],
            "key_fields": []
            if not optiondict.get("key_fields")
            else optiondict["key_fields"],
            "hyperlink_fields": [],
            "disp_msg": False,
            "disp_fmt": True,
            "format_output": True,
    ...
    # if there is no work to do  - then do not format the output file
    if not (
        optiondict.get("col_width")
        or optiondict.get("col_hidden")
        or optiondict.get("format_auto")
        or optiondict["col_format"]
    ):
        if disp_msg:
            print(
                "no formatting applied - no formatting specified in col_width, col_hidden, format_auto, col_format"
            )
        return
    # messaging
    ...
    kv_excel.format_xlsx_with_filter_and_freeze(
        ...
        col_width=optiondict["col_width"],
        col_hidden=optiondict["col_hidden"],
        col_format=optiondict.get("col_format", None),
        disp_msg=disp_msg,
    )
    ...
    if optiondict.get("disp_fmt"):
        print(
            "Applied formatting from: "
            + ",".join(optiondict["conf_json"])
            + " to file:"
        )
...
def format_cell(optiondict):
    ...
    src_lookup = kvxls.create_multi_key_lookup_excel(
        excel_dict_src, optiondict["key_fields"]
    )
...
if __name__ == "__main__":
    ...
    if missing_cols:
        ...
        if optiondict["disp_msg"]:
            print(
                "You are MOST LIKELY reading in the wrong src sheet as we can not find column(s): ",
                "|".join(missing_cols),
            )
            print("Columns we have found are: ", "|".join(src_data[0].keys()))
        sys.exit(1)
    ...
    if missing_cols:
        ...
        if optiondict["disp_msg"]:
            print(
                "You are missing in src_data the following copy_fields column(s): ",
                "|".join(missing_cols),
            )
        sys.exit(1)

    # check that we have the internal_compare_fields  columns
    missing_cols = validate_missing_columns(
        src_data, optiondict, "internal_compare_fields"
    )
    if missing_cols:
        # output messages
        if optiondict["disp_msg"]:
            print(
                "You are missing in src_data the following internal_compare_fields column(s): ",
                "|".join(missing_cols),
            )
        sys.exit(1)
    ...
    # check that we have the internal_compare_fields  columns
    missing_cols = validate_missing_columns(
        dst_data, optiondict, "internal_compare_fields"
    )
    if missing_cols:
        # output messages
        if optiondict["disp_msg"]:
            print(
                "You are missing in dst_data the following internal_compare_fields column(s): ",
                "|".join(missing_cols),
            )
        sys.exit(1)
    ...
    if optiondict["set_blank_fields"]:
        default_recs = kvutil.set_blank_field_values(
            src_data, optiondict["set_blank_fields"]
        )
    else:
        default_recs = 0

    # now take the action
    matched_recs, updated_recs, src_lookup = src_to_dst_actions(
        src_data, dst_data, optiondict
    )

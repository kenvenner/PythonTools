...
def _generate_uid(msgid):
    return (
        datetime.datetime.today().strftime("%Y-%m-%d-%H-%M-%S") + "-msg-" + str(msgid)
    )
...
def init(email_setting):
    ...
    if missing_count:
        if verbose:
            logger.info("init:missing count: %s %s", missing_count, " - return 0")
        return 0
    ...
    if not skip_login:
        # debugging
        if verbose:
            logger.info("init:login and go to folder: %s", email_setting["imap_folder"])
        # if the login failed - return zero
        if not login(mail, email_setting["user"], email_setting["password"]):
            if debug:
                print("init:not_skip_login:failed to login")
            return 0
        else:
            ...
            if email_setting["imap_folder"]:
                select_folder(mail, email_setting["imap_folder"])
                return mail
            else:
...
def move_message(mail, uid, folder):
    ...
    try:
        ...
        if str(e) == "COPY command error: BAD [b'Could not parse command']":
            logger.info(
                "move_message:to_folder: %s %s %s", folder, ":Exception: Invalid folder"
            )
        else:
            logger.info(
                "move_message:to_folder: %s %s %s", folder, ":Exception:", str(e)
            )
        return 0
...
def delete_message(mail, uid):
    try:
        ...
        if False:
            ...
            rv, data = mail.uid("STORE", uid, "+X-GM-LABELS", "\\Trash")
...
def get_next_imap_msg(mail, mail_ids):
    ...
    if mparse:
        logger.info(
            "get_next_imap_msg: pull off first time and return the remainder of the list"
        )
...
def get_imap_msg(mail, msgid):
    ...
    try:
        ...
        # debugging
        logger.info(
            "get_imap_msg:raw_email-type-bytes: %s", isinstance(raw_email, bytes)
        )
        logger.info("--------------------------------------------")
...
def save_body_msg(mparse, basedir="msg"):
    ...
    if mparse.text_html:
        ...
        # debugging - display the file just created
        logger.info(
            "save_body_msg:text_html:outfile: %s", os.path.join(msg_dir, filename)
        )
    ...
    if mparse.text_plain:
        ...
        # debugging - display the file just created
        logger.info(
            "save_body_msg:text_plain:outfile: %s", os.path.join(msg_dir, filename)
        )
...
def read_email_from_gmail(SMTP_SERVER, FROM_EMAIL, FROM_PWD, DIR_ATTACH, MAIL_FOLDER):
    try:
        # debugging
...
class GmailImap:
    ...
    def __init__(self, email_setting={}):
        ...
        self.mparse = (
            None  # the object model for the content of the last parsed MIME object
        )
        self.user = None
        self.password = None
        self.msgPath = (
            None  # filepath to the root directory where messages are saved (passed in)
        )

        # calculated when parsing/procesing a message
        self.msgParsed = False  # defines if a message has been parsed
        self.msgProcessed = (
            None  # defines if the message was successfully parsed/processed
        )
        self.msgSaved = (
            False  # defines if this message was successfully saved to the file system
        )
        ...
        self.imap_server = _setting_or_default(
            "imap_server", email_setting, "imap.gmail.com"
        )

        # capture the folder we are reading from or set a default
        self.folder_read = quote_string_containing_spaces(
            _setting_or_default("imap_folder", email_setting, "inbox")
        )

        # capture the folder we are reading from or set a default
        self.folder_pass = quote_string_containing_spaces(
            _setting_or_default("imap_folder_pass", email_setting, None)
        )

        # capture the folder we are reading from or set a default
        self.folder_fail = quote_string_containing_spaces(
            _setting_or_default("imap_folder_fail", email_setting, None)
        )
        ...
        try:
            # debugging
            if self.verbose:
                print("GmailImap:init:create_imap_obj")
            # create an IMAP object
        ...
        if self.loggedIn and self.folder_read:
            # debugging
            if self.verbose:
                print("GmailImap:select_folder:", self.folder_read)
            self.select_folder()
    ...
    def setIncludeExclude(
        self,
        fromIncludeEmails=[],
        fromIncludeDomains=[],
        fromExcludeIfNotInclude=False,
        fromExcludeEmails=[],
        fromExcludeDomains=[],
    ):
        ...
        for email_addr in fromIncludeEmails:
            self.fromIncludeEmails.append(email_addr.lower())
        ...
        for email_addr in fromExcludeEmails:
            self.fromExcludeEmails.append(email_addr.lower())
    ...
    def login(self, newuser=None, newpassword=None, newfolder=""):
        ...
        if newuser:
            # debugging
            if self.verbose:
                print("GmailImap:login:newuser:", newuser)
            self.user = newuser
        if newpassword:
            # debugging
            if self.verbose:
                print("GmailImap:login:newpassword:", newpassword)
            self.password = newpassword
        if newfolder:
            # debugging
            if self.verbose:
                print("GmailImap:login:newfolder:", newfolder)
        ...
        if rv != "OK":
            ...
            if self.verbose:
                print("GmailImap:login:errmsg:", self.errmsg)
            if self.verbose:
                print("GmailImap:login:data:", data)
            return self.errmsg
        ...
        if self.folder_read:
            # debugging
            if self.verbose:
                print("GmailImap:login:select_folder:", self.folder_read)
            self.select_folder()
    ...
    def select_folder(self, newfolder=""):
        ...
        if not self.loggedIn:
            # debugging
            if self.verbose:
                print("GmailImap:select_folder:not logged in")
            self.error = None
        ...
        if newfolder:
            # debugging
            if self.verbose:
                print("GmailImap:select_folder:newfolder:", newfolder)
            self.folder_read = quote_string_containing_spaces(newfolder)
        ...
        try:
            ...
            if self.verbose:
                print(
                    "GmailImap:select_folder:set-folder-to:self.folder_read:",
                    self.folder_read,
                )
            ...
            if rv != "OK":
                ...
                if self.verbose:
                    print("GmailImap:select_folder:errmsg:", self.errmsg)
                return self.errmsg
            ...
            if str(e) == "SELECT command error: BAD [b'Could not parse command']":
                self.errmsg = (
                    "select_folder:could not select folder:" + self.folder_read
                )
            else:
                self.errmsg = "select_folder:Exception:" + str(e)
            if self.verbose:
                print("GmailImap:select_folder:", self.errmsg)
            return self.errmsg
        ...
        # debugging
        if self.verbose:
            print("GmailImap:select_folder:search_messages")
    ...
    def search_messages(self):
        ...
        try:
            # debugging
            if self.verbose:
                print("GmailImap:search_messages:uid('search')")
            rv, data = self.imapobj.uid(
                "search", None, "ALL"
            ...
            if self.verbose:
                print(
                    "GmailImap:search_messages:folder_current:",
                    self.folder_current,
                    "\nGmailImap:search_messages:rv:",
                    rv,
                    "\nGmailImap:search_messages:data:",
                    data,
                )
            ...
            # debugging
            if self.verbose:
                print("GmailImap:search_messages:mail_ids:", self.mail_ids)
    ...
    def getNextMessage(self):
        ...
        # debugging
        if self.verbose:
            print("GmailImap:getNextMessage:search_messages")
        ...
        # debugging
        if self.verbose:
            print("GmailImap:getNextMessage:mail_ids:", self.mail_ids)

        # if there are messages
        if self.mail_ids:
            # debugging
            if self.verbose:
                print("GmailImap:getNextMessage:mailID:", self.mail_ids[0])
            # set up other flags
            self.clearMsgFlags()
            ...
            # debugging
            if self.verbose:
                print("GmailImap:getNextMessage:mparse:", self.mparse)
            if self.verbose:
                print("GmailImap:getNextMessage:mparse:type:", type(self.mparse))

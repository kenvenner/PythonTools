...
# - double quoted
rePgmVerLineDblQ = re.compile(r"(__version__\s*=\s*\"((\d+)\.(\d+))\")")
reAtVerLineDblQ = re.compile(r"(\@version\s*:\s*((\d+)\.(\d+)))")
reAtQuoteVerLineDblQ = re.compile(r"(\@version\s*:\s*\"((\d+)\.(\d+))\")")
reAppVerLineDblQ = re.compile(r"(AppVersion\s*=\s*\"((\d+)\.(\d+))\")")
reBuildVerLineDblQ = re.compile(r"(BuildVersion\s*=\s*\"(\d+)\")")

reOptVerLineDblQ = re.compile(r"\"AppVersion\"\s*:\s*{")
reOptValueLineDblQ = re.compile(r"^\s+(\"value\"\s*:\s*\"((\d+)\.(\d+))\")")
...
fmtOptValueDblQ = '"value": "{}"'
fmtAtVerLineDblQ = "@version: {}"
fmtAtQuoteVerLineDblQ = '@version: "{}"'
fmtAppVerLineDblQ = 'AppVersion = "{}"'
fmtPgmVerLineDblQ = '__version__ = "{}"'

fmtBuildVerLineDblQ = 'BuildVersion = "{}"'
...
SEARCH4VERSION = [
    ...
    (reAppVerLineDblQ, fmtAppVerLineDblQ, "AppVerLineDblQ"),
    (rePgmVerLineDblQ, fmtPgmVerLineDblQ, "PgmVerLineDblQ"),
    (reAtVerLineDblQ, fmtAtVerLineDblQ, "AtVerLineDblQ"),
    (reAtQuoteVerLineDblQ, fmtAtQuoteVerLineDblQ, "AtQuoteVerLineDblQ"),
]

SEARCH4OPTVERSION = [
    (reOptVerLine, None, "OptVerLine"),
    (reOptVerLineDblQ, None, "OptVerLineDblQ"),
]

SEARCH4OPTVALUE = [
    (reOptValueLine, fmtOptValue, "OptValueLine"),
    (reOptValueLineDblQ, fmtOptValueDblQ, "OptValueLineDblQ"),
]

SEARCH4BUILDVERSION = [
    (reBuildVerLine, fmtBuildVerLine, "reBuildVerLine"),
    (reBuildVerLineDblQ, fmtBuildVerLineDblQ, "reBuildVerLineDblQ"),
]
...
def build_version_update(
    line: str,
    bld_version_found: bool,
    new_bld_version: str,
    debug: bool = False,
) -> tuple[str, bool, int | None, int | None, str]:
    """
    ...
    for reval, fmtval, name in SEARCH4BUILDVERSION:
        if debug:
            print("checking: " + name)
...
def major_minor_version_update(
    line: str,
    new_app_ver: str,
    version_found: bool,
    opt_ver_found: bool,
    ...
    # we are not looking for the version line that goes with this already found line
    if opt_ver_found:
        ...
        for reval, fmtval, name in SEARCH4OPTVALUE:
            if debug:
                print("checking optvalue: " + name)
            ...
            if m:
                ...
                # given we have a match - we are done and bust out of the loop
                break
    ...
    for reval, fmtval, name in SEARCH4OPTVERSION:
        if debug:
            print("checking optver: " + name)
        ...
        if m:
            ...
            if debug:
                print(
                    f"{name} - found and now looking for the value tied to this variable"
                )
                print(f"{line=}")
                print(f"{version_changed=}")
                print(f"{opt_ver_found=}")
    ...
    for reval, fmtval, name in SEARCH4VERSION:
        ...
        if m:
            ...
            if not version_found:
                ...
                # debugging
                if debug:
                    print("Match found and version_found NOT set")
            elif debug:
                print("version_found...")
        ...
        for line in filelines:
            ...
            if buildonly:
                ...
                (
                    ...
                    bld_version_new,
                    new_bld_version,
                ) = build_version_update(
                    line, bld_version_found, new_bld_version, debug=debug
                )
            ...
            (
                ...
                opt_ver_found,
                new_app_ver,
                line_version_changed,
            ) = major_minor_version_update(
                line,
                new_app_ver,
                version_found,
                opt_ver_found,
...
def git_modified_files_in_folder(folder_path="", debug=False):
    ...
    for line in result.stdout.decode("utf-8").split("\n"):
        ...
        if line == untracked:
            if debug:
                print(
                    "git_modified_files_in_folder:encountered the untracked line, files modified list is:",
                    files_modified,
                )
...
def version_changed_in_git_branch(filename, debug=False):
    ...
    for line in result.stdout.decode("utf-8").split("\n"):
        ...
        if linecnt > MAXLINES_CHECKED:
            if debug:
                print("Maxline met before finding field of interest")
            return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input_file", nargs="*", help="file we are updating the version on"
    )
    parser.add_argument(
        "--input_folder",
        default="./",
    ...
    parser.add_argument("--input_list", help="list of files to be processed")
    parser.add_argument("--input_glob", help="file glob files to be processed")
    parser.add_argument(
        "--major_update",
        "--major",
        "--major_ver",
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version="%(prog)s " + __version__ + "." + BuildVersion,
    )
    ...
    if filelist:
        ...
        files_to_check = git_modified_files_in_folder(
            vargs["input_folder"], debug=vargs["debug"]
        )
